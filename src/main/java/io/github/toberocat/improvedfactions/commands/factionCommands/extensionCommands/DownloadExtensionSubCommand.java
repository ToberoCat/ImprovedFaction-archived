                        ...
                        ExtensionDownloader.DownloadExtension(extension,
                                ImprovedFactionsMain.getPlugin().getDataFolder().getPath() + "/Extensions", new ExtensionDownloadCallback() {
                                    ...
                                    public void FinishedDownload(ExtensionObject extension) {
                                        player.sendMessage(Language.getPrefix() + "§fFinished download for §e" + extension.getName() + ". §fTo enable it, reload the server using /rl");
                                    }
                                });
                    } catch (IOException e) {
        ...
        if (results.size() == 1) {
            if (extension != null) {
                if (lastExtensionObj != extension) {
                    player.sendMessage("§e§l"+extension.getName() + ": §f" + extension.getDescription());
