...
import io.github.toberocat.improvedfactions.factions.FactionFlag;
...
public class Flag<T> {
    ...
    public ItemStack ToggleSetting(Faction faction, Player player) {
        switch (type) {
            ...
            case Boolean:
                ...
                return getItem();
            case Function:
                callback.CallBack(faction, player);
                return getItem();
            default:
                return getItem();
        }
    ...
    private static Flag getFlag(String name) {
        for (FactionFlag flag : Faction.FLAGS) {
            if (flag.getId().equals(name)) return flag.getFlag();
        }
        return null;
    }
