...
public class FactionRanksGui implements Gui {
    ...
    public FactionRanksGui(Player player, Faction faction) {
        ...
        page = 0;
        maxPage = 0;
        AddPermissionFlag(Material.BRICKS, "&aBuild permission", faction.getPermission(Faction.BUILD_PERMISSION));
        AddPermissionFlag(Material.WOODEN_PICKAXE, "&aBreak permission", faction.getPermission(Faction.BREAK_PERMISSION));
        AddPermissionFlag(Material.SHIELD, "&aClaim permission", faction.getPermission(Faction.CLAIM_CHUNK_PERMISSION));
        AddPermissionFlag(Material.WOODEN_AXE, "&aUnclaim permission", faction.getPermission(Faction.UNCLAIM_CHUNK_PERMISSION));
        AddPermissionFlag(Material.NAME_TAG, "&aInvite permission", faction.getPermission(Faction.INVITE_PERMISSION));

        openInventory(player, faction);
    }

    private void AddPermissionFlag(Material material, String flagName, FactionRankPermission permissions) {
        AddToSlot(new PermissionFlag(flagName, material, permissions));
    }
