    ...
    public static void register() {
        ...
        add(new CallbackSettings((player) -> {
            ...
            new AnvilGUI.Builder().onClose((user) -> {
                    ...
                    .plugin(MainIF.getIF())
                    .open(player);
        },"change?motd", "Input", Utility.createItem(Material.OAK_SIGN, "&eSet Motd", new String[]{"&8Change your motd",
                "&7Tip: Use color codes"})));
        add(new CallbackSettings((player) -> {
            Faction faction = FactionUtility.getPlayerFaction(player);
            if (faction == null) return;

            AsyncCore.runLaterSync(0, () -> new MemberGui(player, faction, new GUISettings()
        ...
        add(new CallbackSettings((player) -> {
            ...
            AsyncCore.runLaterSync(0, () -> new RankGui(player, faction, new GUISettings()
